...
export const register = (req, res) => {
  ...
  User.findOne({
    ...
    .then((user) => {
      if (user) {
        if (user.username === newUserData.username)
          res.status(400).json({
            message: {
              messageBody: "Username already in use",
              messageError: true,
            },
          });
          ...
          res.status(400).json({
            message: {
              messageBody: "Email already in use",
              messageError: true,
            },
          });
      } else {
        ...
        newUser
          ...
          .then((user) => {
            res.status(201).json({
              message: {
                messageBody: "Account succesfully created",
                messageError: false,
              },
            });
          })
          .catch((err) => {
            ...
            res.status(500).json({
              message: {
                messageBody: "Error has occured (saving user)",
                messageError: true,
              },
            });
          });
    ...
    .catch((err) => {
      res.status(500).json({
        message: {
          messageBody: "Error has occured (checking if user exists)",
          messageError: true,
        },
      });
    });
...
export const login = (req, res) => {
  if (req.isAuthenticated()) {
    ...
    res
      .status(200)
      .json({ isAuthenticated: true, user: { username, privilege } });
  }
};
...
export const admin = (req, res) => {
  if (req.user.privilege === "admin") {
    res.status(200).json({
      message: { messageBody: "You are an admin", messageError: false },
    });
  } else
    res.status(403).json({
      message: { messageBody: "You are not an admin", messageError: true },
    });
};
...
export const getUser = (req, res) => {
  User.findOne({ username: req.params.username })
    .then((selectedUser) => {
      if (!selectedUser)
        return res.status(403).json({
          message: { messageBody: "Error, no user", messageError: true },
        });
    ...
    .catch((err) => {
      return res.status(403).json({
        message: {
          messageBody: "Error getting user from DB",
          messageError: true,
        },
      });
    });
...
export const adminUpdateUser = (req, res) => {
  ...
  if (req.body.privilege != "admin" && req.body.privilege != "user")
    res.status(400).json({
      message: {
        messageBody: "Privilege must be user or admin",
        messageError: true,
      },
    });
  ...
  User.findOne({
      ...
      if (user && user._id != updatedUserData._id) {
        ...
        User.findByIdAndUpdate(
          ...
          .then((user) => {
            res.status(201).json({
              message: {
                messageBody: "User succesfully updated",
                messageError: false,
              },
            });
          })
          .catch((err) => {
            ...
            res.status(500).json({
              message: {
                messageBody: "Error has occured (updating user)",
                messageError: true,
              },
            });
          });
    ...
    .catch((err) => {
      ...
      res.status(500).json({
        message: {
          messageBody: "Error has occured (Checking username/email)",
          messageError: true,
        },
      });
    });
